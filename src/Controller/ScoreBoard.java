...
public class ScoreBoard extends StackPane {
	...
	public static void addScore(HumanSprite enemy) {
		if (enemy instanceof BadHuman) {
			ScoreBoard.addScore(BadHuman.SCORE);
		} else if (enemy instanceof SwordMan) {
			ScoreBoard.addScore((SwordMan.SCORE));
		} else if (enemy instanceof TrapMan) {
			ScoreBoard.addScore(TrapMan.SCORE);
		} else {
			ScoreBoard.addScore(GunMan.SCORE);
		}
